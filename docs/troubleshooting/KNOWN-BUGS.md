...
- ⚪ Still true: OS baseline not applied (avahi EACCES, `uv_os_setpriority` EACCES, SSH password unset)
  — operator runs `sudo bash scripts/node-baseline/apply-baseline.sh` on the node. The 100 ms mic
  level probe on the audio page still times out at 2 s on this 4 GB Pi and logs it each time.
