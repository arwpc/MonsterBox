...
export const streamMJPEG = async (req, res) => {
  try {
    ...
    // Proxy the stream from mjpg-streamer using http.get
    const streamReq = http.get(MJPG_STREAM_ENDPOINT, (streamRes) => {
      ...
      streamRes.on('error', (error) => {
        console.error('mjpg-streamer stream error:', error);
    ...
    streamReq.on('error', (error) => {
      console.error('Error connecting to mjpg-streamer:', error);
      try { _activeVideoUse.delete(devicePath); } catch (_) { }
      if (!res.headersSent) {
        res.status(503).json({
          success: false,
          error: 'Failed to connect to mjpg-streamer service: ' + error.message
        });
      }
    });
