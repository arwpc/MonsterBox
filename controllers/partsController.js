...
export const createPart = async (req, res) => {
    try {
    const { name, type, pin, description, config, directionPin, pwmPin, leftEnablePin, rightEnablePin, maxExtension, maxRetraction } = req.body;
        ...
        if (type === 'linear_actuator') {
            ...
            // 1) Legacy single directionPin + pwmPin (or base pin derivation)
            // 2) BTS7960 dual enable: leftEnablePin + rightEnablePin + pwmPin
            const hasDualEnable = (leftEnablePin != null && rightEnablePin != null && pwmPin != null);
            const hasLegacy = (directionPin != null && pwmPin != null);

            if (!hasDualEnable && !hasLegacy) {
                // Attempt derivation from base pin for legacy pattern
                if (pwmPin == null && (typeof pin === 'number' || (typeof pin === 'string' && pin !== ''))) {
                    req.body.directionPin = typeof pin === 'number' ? pin : parseInt(pin, 10);
                    req.body.pwmPin = (typeof pin === 'number' ? pin + 1 : parseInt(pin, 10) + 1);
                } else {
                    return res.status(400).json({
                        success: false,
                        error: 'Provide either (directionPin & pwmPin), (leftEnablePin & rightEnablePin & pwmPin), or a base pin for motor parts'
                    });
                }
            }
        ...
        if (type === 'motor') {
            const hasDualEnable = (req.body.leftEnablePin != null && req.body.rightEnablePin != null && (req.body.pwmPin != null || pwmPin != null));
            if (hasDualEnable) {
                newPart.leftEnablePin = parseInt(req.body.leftEnablePin, 10);
                newPart.rightEnablePin = parseInt(req.body.rightEnablePin, 10);
                newPart.pwmPin = parseInt(req.body.pwmPin != null ? req.body.pwmPin : pwmPin, 10);
            } else {
                const dirPin = (req.body && req.body.directionPin != null) ? req.body.directionPin : directionPin;
                const pwmP = (req.body && req.body.pwmPin != null) ? req.body.pwmPin : pwmPin;
                newPart.directionPin = parseInt(dirPin, 10);
