...
# MonsterBox 5.0 Complete Installation Script
...
print_status "Installing MonsterBox 5.0 for user: $ACTUAL_USER"
...
# 5b. Install additional Python packages via pip (fallback for missing system packages)
print_status "Installing additional Python packages via pip..."
python3 -m pip install --break-system-packages \
    smbus2 \
    lgpio

# 5c. Install Python packages from requirements.txt if available
REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "$REPO_DIR/utils/requirements.txt" ]; then
    print_status "Installing Python packages from requirements.txt..."
    python3 -m pip install --break-system-packages -r "$REPO_DIR/utils/requirements.txt" || print_warning "Some packages from requirements.txt may have failed to install"
else
    print_warning "utils/requirements.txt not found, skipping additional Python packages"
fi
...
NEW_CHAR_NAME=$(hostname)
print_status "Using hostname '$NEW_CHAR_NAME' as character name"
...
print_success "MonsterBox 5.0 system installation complete!"
