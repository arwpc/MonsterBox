...
class EnhancedServiceManager extends EventEmitter {
    ...
    async startServiceProcess(serviceName, registration, config) {
        return new Promise((resolve, reject) => {
            ...
            // Infer execution type from script extension or service type
            let executionType = type;
            if (type === 'hardware' && script) {
                // For hardware services, infer execution type from script extension
                executionType = script.endsWith('.py') ? 'python' : 'node';
            }
            ...
            } else if (executionType === 'node') {
                command = 'node';
                processArgs = [script, ...args];
            } else if (type === 'hardware') {
                // Hardware services without scripts are handled by unified hub
                logger.info(`⏭️ Skipping ${serviceName} - hardware service handled by unified hub`);
                const mockProcess = {
                    pid: null,
                    killed: false,
                    on: () => { },
