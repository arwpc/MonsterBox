...
# FIXED VERSION - Safer process killing to prevent kernel panics
set -euo pipefail
...
# Kill anything holding our port - but do NOT kill mpv/vlc as it causes kernel panics
# On a fresh boot, these shouldn't be running anyway
lsof -ti:3001 | xargs -r kill -9 2>/dev/null || true

# REMOVED: pkill -f mpv - causes kernel panic on some Pis
# REMOVED: pkill -f vlc - not needed, we don't use VLC
