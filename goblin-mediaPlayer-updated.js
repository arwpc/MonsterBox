...
class MediaPlayer {
  constructor(goblinServer) {
    ...
    // Video settings (will use goblin.settings if available)
    this.framerate = 60;
  ...
  async playVideo(filename, options = {}) {
    try {
      ...
      if (selectedPlayer === 'mpv') {
        ...
        // Get settings from goblin server or use defaults
        const resolution = this.goblin.settings?.resolution || this.resolution || '1280x720';
        const framerate = this.goblin.settings?.framerate || this.framerate || 60;
        const drmMode = `${resolution}@${framerate}`;
        ...
        const mpvArgs = [
          ...
          `--drm-mode=${drmMode}`,          // Use configured resolution and framerate
        ...
        console.log(`🎬 Starting mpv with hardware acceleration on ${hdmiPort} @ ${drmMode}`);
      ...
      // Only attach stdout/stderr handlers if streams are available
