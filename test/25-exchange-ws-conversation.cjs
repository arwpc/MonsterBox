#!/usr/bin/env node
/**
 * MonsterBox 5.3 - 25 Exchange Real-Time Conversation Test
 * Simulates a complete 25-exchange conversation with Orlok
 * Tests TTS → Speaker → Microphone → STT → AI Agent pipeline
 */
            ...
            try {
                ...
                switch (message.type) {
                    case 'connected':
                        ...
                        // Set character ID (Orlok = character 3)
                        ws.send(JSON.stringify({
                            type: 'set_character',
                            characterId: 3
                        }));

                        // Set output mode to server (play through speaker)
                        ws.send(JSON.stringify({
