...
Write-Host "🤖 Starting Agent 1: AI Integration Testing on Orlok RPI4b" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "Remote Target: Count Orlok (192.168.8.120)" -ForegroundColor Yellow
Write-Host "Focus: http://192.168.8.120:3000/ai-integration-test.html" -ForegroundColor Yellow
...
$env:TEST_URL = "http://192.168.8.120:3000/ai-integration-test.html"
...
$deployScript = @"
#!/bin/bash
echo '🧛 Agent 1: Setting up MonsterBox on Orlok RPI4b'
...
scp deploy-agent-1.sh remote@192.168.8.120:/tmp/
ssh remote@192.168.8.120 "chmod +x /tmp/deploy-agent-1.sh && /tmp/deploy-agent-1.sh"

Write-Host ""
Write-Host "🎯 Agent 1 Remote Testing Instructions:" -ForegroundColor Magenta
Write-Host "1. Application running on: http://192.168.8.120:3000/ai-integration-test.html" -ForegroundColor White
Write-Host "2. Test AI chat with all characters using Orlok's hardware" -ForegroundColor White
Write-Host "3. Verify TTS generation with actual audio output on RPI4b" -ForegroundColor White
Write-Host "4. Test WebSocket connections and real-time features" -ForegroundColor White
...
Write-Host "8. Test AI character management and conversation history" -ForegroundColor White
Write-Host "9. Update tests based on hardware testing results" -ForegroundColor White
Write-Host "10. Commit fixes to agent-1-ai-fixes branch" -ForegroundColor White
Write-Host ""
...
Write-Host "View logs: ssh remote@192.168.8.120 'tail -f /home/remote/MonsterBox/log/*.log'" -ForegroundColor White
