  ...
  before(async function () {
    ...
    if (hasData) {
      try {
        ...
        hasMeasuredCalibration = Object.entries(store).some(([key, p]) =>
          key.startsWith(`${REAL_CHAR}:`) && p && p.autoGenerated !== true && p.bounds);
      } catch (_) { /* stays false */ }
      if (!hasMeasuredCalibration) {
        console.log(`[gesture-engine.test] character ${REAL_CHAR} has no measured calibration window on this machine — the no-rejected-recipes canary is skipped (it runs on the animatronic's own gate)`);
      }
    }
  });
