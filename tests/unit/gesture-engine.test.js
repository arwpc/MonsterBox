...
describe('Gesture engine — real character data', function () {
  ...
  let engine;
  let hasData = false;
  let hasMeasuredCalibration = false;
  const REAL_CHAR = 3;
  ...
  before(async function () {
    ...
    // The no-rejected-recipes canary validates the vocabulary against this
    // node's MEASURED calibration windows, and data/calibration_profiles.json
    // is node-local (never committed, never deployed). On the animatronic that
    // canary MUST run — the gate runs there on every push. On a machine that
    // only has the repo (CI, cloud), the store holds placeholders at best,
    // every raw-target recipe is rejected for "no calibrated bounds", and a
    // failure would report the ENVIRONMENT, not the vocabulary. The pure-data
    // checks below (two-parts rule, blocked parts, rail overlap) keep running
    // everywhere — they read only committed files.
    //
    // "Measured" has to mean measured FOR THE PARTS THE RECIPES TARGET. After
    // the 2026-09-06 fleet-wide calibration wipe, this node's store held one homed
    // actuator (3:1, autoGenerated:false, bounds {minP:0}) and nothing for the
    // head and forearm servos the vocabulary actually moves, so "some profile
    // is measured" armed the canary and every recipe failed for a reason that
    // had nothing to do with the vocabulary. Arm it only when every raw-target
    // part has a real two-ended window.
    hasMeasuredCalibration = false;
    if (hasData) {
      try {
        ...
        const rawTargetParts = new Set();
        for (const g of recipes) {
          for (const st of (g && g.steps) || []) {
            if (st && st.partId != null && st.target != null) rawTargetParts.add(String(st.partId));
          }
        }
        const measured = (pid) => {
          const p = store[`${REAL_CHAR}:${pid}`];
          if (!p || p.autoGenerated === true || !p.bounds) return false;
          const lo = p.bounds.minAngle ?? p.bounds.minP ?? p.bounds.min;
          const hi = p.bounds.maxAngle ?? p.bounds.maxP ?? p.bounds.max;
          return lo != null && hi != null && Number(hi) > Number(lo);
        };
        const unmeasured = [...rawTargetParts].filter((pid) => !measured(pid));
        hasMeasuredCalibration = rawTargetParts.size > 0 && unmeasured.length === 0;
        if (!hasMeasuredCalibration) {
          console.log(`[gesture-engine.test] character ${REAL_CHAR}: raw-target part(s) ${unmeasured.join(', ') || '(none)'} have no measured calibration window on this machine — the no-rejected-recipes canary is skipped (it runs on the animatronic's own gate once those parts are calibrated)`);
        }
      } catch (_) { /* stays false */ }
    }
  });
  ...
  it('loads the shipped vocabulary with no rejected recipes', async function () {
    if (!hasData || !hasMeasuredCalibration) this.skip();
