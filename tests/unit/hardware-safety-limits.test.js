...
describe('Hardware safety limits', function () {
  ...
  describe('angle clamping', function () {
    ...
    it('ignores an autoGenerated placeholder profile rather than trusting it as calibration', function () {
      // The store stamps every uncalibrated absolute-servo with a full-span 0-180
      // placeholder. Believing it means believing a part is bounded when nothing
      // has measured it.
      const result = applySafetyLimits({
        type: 'servo',
        action: 'moveToAngle',
        params: { angleDeg: 150 },
        profile: { autoGenerated: true, bounds: { minAngle: 0, maxAngle: 180 } },
        safety: { maxAngle: 120 },
        partId: 15
      });
      // Only the configured limit applies; the placeholder contributes nothing.
      expect(result.params.angleDeg).to.equal(120);
    });

    it('still honours a genuinely calibrated profile', function () {
      const result = applySafetyLimits({
        type: 'servo',
        action: 'moveToAngle',
        params: { angleDeg: 150 },
        profile: { autoGenerated: false, bounds: { minAngle: 45, maxAngle: 135 } },
        safety: {},
        partId: 4
      });
      expect(result.params.angleDeg).to.equal(135);
