...
describe('Multi-turn servo real-degree path', function () {
  ...
  it('re-typing a part rebuilds its PLACEHOLDER profile kind (bench continuous-click recovery)', async function () {
    // The standard synthetic part still carries an autoGenerated profile. A
    // stray servoType flip must rebuild the placeholder in BOTH directions —
    // this is exactly how the knight's head got stuck refusing angle commands.
    await writeParts(parts => {
      const p = parts.find(x => String(x.id) === String(STD_PART_ID));
      p.config.servoType = 'continuous';
    });
    let res = await request(app).get(`/api/calibration/${STD_PART_ID}/profile`).expect(200);
    expect((res.body.profile || res.body).capability.kind).to.equal('continuous-servo');

    await writeParts(parts => {
      const p = parts.find(x => String(x.id) === String(STD_PART_ID));
      p.config.servoType = 'multi-turn';
      p.config.rotationRangeDeg = 900;
    });
    res = await request(app).get(`/api/calibration/${STD_PART_ID}/profile`).expect(200);
    const prof = res.body.profile || res.body;
    expect(prof.capability.kind).to.equal('absolute-servo');
    expect(prof.capability.maxAngleDeg).to.equal(900);

    await writeParts(parts => {
      const p = parts.find(x => String(x.id) === String(STD_PART_ID));
      p.config.servoType = 'standard';
      delete p.config.rotationRangeDeg;
    });
  });

  it('release de-energizes the part channel (plug/unplug safety)', async function () {
    // A live channel holds its last pulse, so plugging a servo lead in slams
    // it to that stale position — the knight's head found this the hard way.
    // In CI the daemon is disabled, so the endpoint must fall back to the
    // one-shot wrapper and the simulated exec echoes the release args.
    const res = await request(app)
      .post(`/api/calibration/${MULTI_PART_ID}/release`)
      .send({})
      .expect(200);
    expect(res.body.success).to.equal(true);
    expect(res.body.released).to.equal(true);
    expect(res.body.channel).to.equal(14);
    expect(res.body.via).to.equal('servo_cli');
  });

  it('a lost servoType cannot demote a ranged part to the 0-180 path (drift honesty)', async function () {
    // servoType and rotationRangeDeg are two records of one physical fact, and
    // they HAVE drifted on a live node (an overrides save deleted 'multi-turn'
    // while the profile still accepted 0-900 targets): the standard path then
    // mapped real 450 to full-scale pulse — real 900 into the head cabling.
    // Dispatch must treat a declared non-180 range as multi-turn on its own.
    const { HARDWARE_CONTROLLERS } = (await import('../../services/hardwareService/index.js')).default;
    const result = await HARDWARE_CONTROLLERS.servo.moveToAngle({
      partId: MULTI_PART_ID, channel: 14, angleDeg: 450,
      controllerType: 'pca9685', servoType: 'standard', rotationRangeDeg: 900
    });
    const echoed = JSON.parse(result.rawOutput);
    expect(echoed.args[0], 'declared range wins over a lost/stale type string').to.equal('move_to_pca_multi');
    expect(Number(echoed.args[2])).to.equal(900);

    // ...but an explicit 180 range is NOT a retype: standard stays standard.
    const std = await HARDWARE_CONTROLLERS.servo.moveToAngle({
      partId: STD_PART_ID, channel: 13, angleDeg: 90,
      controllerType: 'pca9685', servoType: 'standard', rotationRangeDeg: 180
    });
    const stdEchoed = JSON.parse(std.rawOutput);
    expect(stdEchoed.args[0]).to.equal('move_to_pca');
  });

  it('an angle on a continuous servo dispatches as a BOUNDED timed spin, never a held pulse', async function () {
    const { HARDWARE_CONTROLLERS } = (await import('../../services/hardwareService/index.js')).default;
    const result = await HARDWARE_CONTROLLERS.servo.moveToAngle({
      partId: STD_PART_ID, channel: 13, angleDeg: 90,
      controllerType: 'pca9685', servoType: 'continuous'
    });
    const echoed = JSON.parse(result.rawOutput);
    expect(echoed.args[0], 'continuous fallback is the timed rotation wrapper').to.equal('rotate_continuous_pca');
    expect(Number(echoed.args[4]), 'the spin must carry a finite duration').to.be.greaterThan(0);
  });

  it('batchMoveServos routes a continuous part around the positional batch path', async function () {
    // The daemon batch writes HELD positional pulses; on a continuous servo
    // that is an indefinite spin. The per-part path converts to a timed spin.
    await writeParts(parts => {
      const p = parts.find(x => String(x.id) === String(STD_PART_ID));
      p.config.servoType = 'continuous';
    });
    try {
      ...
      const { batchMoveServos } = (await import('../../services/hardwareService/index.js')).default;
      const out = await batchMoveServos([
        { partId: String(STD_PART_ID), angleDeg: 90 }
      ], { characterId });
      const r = out.results.find(x => String(x.partId) === String(STD_PART_ID));
      expect(r, 'the continuous part must produce a result').to.exist;
      const echoed = JSON.parse(r.rawOutput);
      expect(echoed.args[0], 'must go through the per-part converting seam, not batch_pca').to.equal('rotate_continuous_pca');
    } finally {
      await writeParts(parts => {
        const p = parts.find(x => String(x.id) === String(STD_PART_ID));
        p.config.servoType = 'standard';
  ...
  it('dir/scale nudges scale with the real range: one "med" press is 25 real degrees on a 900° part', async function () {
    // 2/5/15 are pulse-step-sized for a 180° servo; unscaled they fell inside
    // the 900° gearbox's deadband and every press read as a dead part.
    await request(app)
      .post(`/api/calibration/${MULTI_PART_ID}/goto`)
      .send({ angle: 450, calibrationOverride: true })
      .expect(200);
    const res = await request(app)
      .post(`/api/calibration/${MULTI_PART_ID}/nudge`)
      ...
      .expect(200);
    expect(res.body.success).to.equal(true);
    const landed = Number(res.body.currentAngle ?? res.body.angle);
    expect(landed, 'med = 5 × (900/180) = 25 real degrees').to.equal(475);
  });
