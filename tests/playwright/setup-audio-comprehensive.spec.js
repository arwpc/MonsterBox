...
test.describe('Setup Audio - Comprehensive Button Tests', () => {
  ...
  test('should display VU meters', async ({ page }) => {
    // Check for input VU meter exists
    const inputVU = page.locator('#input-vu-meter');
    await expect(inputVU).toHaveCount(1);

    // Check for output VU meter exists
    const outputVU = page.locator('#output-vu-meter');
    await expect(outputVU).toHaveCount(1);
  });
  ...
  test('Test Audio Output button should work', async ({ page }) => {
    ...
    // Verify #test-results shows speaker test running (UI evidence of command sent)
    await expect(page.locator('#test-results')).toContainText(/Running\s+speaker\s+test/i, { timeout: 5000 });

    // Wait briefly to allow test flow
  ...
  test('Test Audio Input button should work', async ({ page }) => {
    ...
    // Verify #test-results shows microphone test running (UI evidence of command sent)
    await expect(page.locator('#test-results')).toContainText(/Running\s+microphone\s+test/i, { timeout: 5000 });
