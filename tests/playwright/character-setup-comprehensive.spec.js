...
test.describe('Character Setup - Page Load and UI', () => {
  ...
  test('Characters table displays correctly', async ({ page }) => {
    ...
    if (hasTable) {
      // Verify table headers (Playwright strict-mode friendly)
      await expect(page.locator('th')).toContainText(['Avatar', 'Name', 'AI Agent', 'Status', 'Actions']);
...
test.describe('Character Setup - Delete Character', () => {
  ...
  test('Delete character with confirmation', async ({ page }) => {
    ...
    if (rowsBefore > 0) {
      // Prefer deleting a test-created character to avoid protected seeds
      let row = page.locator('tbody tr', { hasText: 'TestChar' }).first();
      if (await row.count() === 0) row = page.locator('tbody tr').first();

      const nameText = (await row.locator('td').nth(2).textContent() || '').trim();

      // Set up dialog handler
      page.on('dialog', dialog => dialog.accept());

      // Click delete button scoped to the target row
      await row.locator('button[onclick*="remove"]').first().click();

      // Wait until the row with this name is gone
      await expect(page.locator('tbody tr', { hasText: nameText })).toHaveCount(0, { timeout: 10000 });

      // Optionally also verify row count does not increase
      const rowsAfter = await page.locator('tbody tr').count();
      expect(rowsAfter).toBeLessThanOrEqual(rowsBefore - 1);

      console.log(`✅ Deleted character "${nameText}"`);
    } else {
...
test.describe('Character Setup - Select Current Character', () => {
  ...
  test('Select character as current', async ({ page }) => {
    ...
    if (await selectBtn.count() > 0) {
      ...
      // Verify "Current" badge appears (non-strict)
      await expect(page.locator('.badge.bg-success').first()).toContainText('Current');
