...
test.describe('Orchestration Control Panel', () => {
    ...
    test('say button should trigger speech', async ({ page }) => {
        ...
        const logEntry = page.locator('.log-entry:has-text("Making")');
    ...
    test('ask AI button should trigger AI response', async ({ page }) => {
        ...
        const aiLogEntry = page.locator('.log-entry:has-text("🤖")');
    ...
    test('refresh button should update status', async ({ page }) => {
        ...
        const refreshLog = page.locator('.log-entry:has-text("Status refreshed successfully")');
    ...
    test('audio controls should be visible on online animatronics', async ({ page }) => {
        ...
        // Verify Play button
        const playButton = onlineCard.locator('button:has-text("Play")');
        await expect(playButton).toBeVisible();
        
        // Verify Stop button
        const stopButton = onlineCard.locator('button:has-text("Stop")');
        await expect(stopButton).toBeVisible();
        
        // Verify Loop checkbox
        const loopCheckbox = onlineCard.locator('input[type="checkbox"]');
    ...
    test('AI chat controls should be visible on online animatronics', async ({ page }) => {
        ...
        const askButton = onlineCard.locator('button:has-text("Ask AI")');
        await expect(askButton).toBeVisible();
        
        // Verify Say button
        const sayButton = onlineCard.locator('button:has-text("Say")');
        await expect(sayButton).toBeVisible();
        
        // Verify text input
        const textInput = onlineCard.locator('input[type="text"]');
...
test.describe('Orchestration API Integration', () => {
    ...
    test('health check API should work', async ({ request }) => {
        const response = await request.post(`${BASE_URL}/api/orchestration/health-check`);
        expect(response.ok()).toBeTruthy();
