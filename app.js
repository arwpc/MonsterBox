...
const app = express();
const port = 3000;
const motorRoutes = require('./routes/motorRoutes'); // added by claude
...
app.use('/motor', motorRoutes);  // Claude's idea
...
// Update the POST route for creating a new scene
app.post('/scenes', async (req, res) => {
    const scenes = await dataManager.getScenes();
    console.log('Received steps:', req.body.steps); // Add this line for debugging
    let parsedSteps;
    try {
    ...
    console.log('New scene object:', newScene); // Add this line for debugging
    scenes.push(newScene);
    await dataManager.saveScenes(scenes);
    res.redirect('/scenes');
});

// Update the POST route for updating an existing scene
app.post('/scenes/:id', async (req, res) => {
    ...
    if (index !== -1) {
        ...
        console.log('Updated scene object:', scenes[index]); // Add this line for debugging
        await dataManager.saveScenes(scenes);
        res.redirect('/scenes');
    } else {
        res.status(404).send('Scene not found');
