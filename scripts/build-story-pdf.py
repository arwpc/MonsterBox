...
def callout_commit():
    ...
    note = Paragraph(
        ...
        "&mdash; all <b>2,469</b> of them &mdash; to reconstruct how the project, and the AI building "
        "it, evolved.", S['callbi'])
...
MONTHS = [("Aug'24",80,20,6,11),("Sep'24",49,17,4,10),("Oct'24",601,13,22,15),
 ...
 ("Mar'26",109,70,0,2),("Apr'26",36,62,0,0),("May–Jun",None,None,None,None),
 ("Jul'26",33,76,0,2),("Aug'26",342,86,0,15),("Sep'26",74,89,0,0)]
...
def chart_activity():
    ...
    for i,(m,c,ln,f,t) in enumerate(MONTHS):
        ...
        lbl(d,L+i*bw+bw/2,10,m,5.1,MUTED,'middle')
...
def chart_msglen():
    ...
    d=Drawing(W,H); pw,ph=W-L-R,H-T-B; mx=100; n=len(MONTHS)
    ...
    for i,(m,c,ln,f,t) in enumerate(MONTHS):
        ...
        lbl(d,xi(i),12,m,5.0,MUTED,'middle')
    lbl(d,xi(2),yy(13)-12,"manual grind",6.5,RED,'middle')
    lbl(d,xi(6),yy(40)+12,"AI writes the commits",6.5,GREEN,'middle')
    lbl(d,xi(14),yy(65)+12,"enforced grammar",6.5,PURPLE,'middle')
    lbl(d,xi(18),yy(76)-11,"findings, not diffs",6.5,HexColor(0x0f766e),'middle')
    lbl(d,L+pw/2,H-6,"avg commit-message length (chars), by month",6.5,MUTED,'middle')
    return d

def chart_trailers():
    """AI co-author trailers resolved to named models — the 2026 provenance record."""
    W,H=DW,232; L,R,T,B=150,14,26,34
    d=Drawing(W,H)
    data=[("Claude Opus 5",255,PURPLE,"Aug 15 - Sep 12"),
          ("Claude Opus 4.6",206,HexColor(0x8b7ad6),"Feb 14 - Apr 14"),
          ("Claude Fable 5",96,HexColor(0x7c3aed),"Aug 16 - Sep 7"),
          ("Claude Opus 4.8",47,HexColor(0x8b7ad6),"Jul 11 - Sep 13"),
          ("Claude (unversioned)",40,GREY,"Feb - Aug 2026"),
          ("Claude Opus 4.7",15,HexColor(0x8b7ad6),"Apr 18 - Apr 19"),
          ("Claude Fable 5.1",15,HexColor(0x7c3aed),"Sep 7"),
          ("Claude Sonnet 4.5",3,HexColor(0xa5b4fc),"Feb 14")]
    pw,ph=W-L-R,H-T-B; n=len(data); rh=ph/n; mx=330
    for gv in (0,50,100,150,200,250,300):
        x=L+(gv/mx)*pw
        d.add(Line(x,B,x,B+ph,strokeColor=GRID,strokeWidth=0.5))
        lbl(d,x,B-9,str(gv),6,MUTED,'middle')
    for i,(nm,v,col,span) in enumerate(data):
        y=B+ph-(i+1)*rh+rh*0.22; h=rh*0.56
        d.add(Rect(L,y,(v/mx)*pw,h,fillColor=col,strokeColor=None))
        lbl(d,L-5,y+h*0.28,nm,6.4,INK,'end')
        lbl(d,L+(v/mx)*pw+4,y+h*0.28,"%d  %s"%(v,span),6,MUTED,'start')
    lbl(d,L+pw/2,H-8,"664 AI-authored commits, by named model (2026) - the trailer became a provenance field",6.5,MUTED,'middle')
    lbl(d,L+pw/2,B-20,"commits carrying a Co-Authored-By trailer",6,MUTED,'middle')
    return d
...
def chart_filecount():
    ...
    d=Drawing(W,H); data=[("v0.1.0","Jun'25",312),("v5.3.0","Oct 8",1889),
     ...
     ("v8.1.0","Apr 14",908),("v8.3.0","Apr 19",921),("v8.4.0","Jul 11",911),
     ("v9.0.0","Aug 16",964),("v10.0.0","Aug 18",1103),("v10.4.0","Aug 19",1128),
     ("v10.5.1","Sep 4",1253)]
    ...
    for i,(tg,dt,v) in enumerate(data):
        ...
        lbl(d,x+w/2,yy(v)+3,str(v),5.8,INK,'middle')
        lbl(d,x+w/2,16,tg,5.8,INK,'middle'); lbl(d,x+w/2,8,dt,5.0,MUTED,'middle')
    lbl(d,L+pw/2,H-6,"tracked files at each release tag",6.5,MUTED,'middle')
    return d

def chart_tools():
    W=DW; L,R,T=170,12,28
    start=2024+7/12; end=2026+9/12; span=end-start
    ...
    groups=[("LLMS (THE MODELS)",PURPLE,[
        ...
        ("Claude Sonnet 4.5 -> Opus 4.6/4.7",2026+1/12,2026+3.7/12,PURPLE,"governed"),
        ("Claude Opus 4.8 -> Opus 5",2026+6.3/12,2026+9/12,PURPLE,"255 commits in 4 wks"),
        ("Claude Fable 5 -> 5.1",2026+7.5/12,2026+9/12,HexColor(0x7c3aed),"brief, do not instruct"),
        ("gpt-oss-120b (in-product)",2026+7.5/12,2026+9/12,HexColor(0x8b7ad6),"the monsters own heads")]),
     ("AGENT HARNESSES / IDEs",AMBER,[
        ...
        ("GitHub Copilot",2025+9/12,2026+9/12,BLUE,""),
        ("Claude Code + CLAUDE.md",2026+1/12,2026+9/12,PURPLE,"governed"),
        ("Subagents / skills / workflows",2026+7.4/12,2026+9/12,HexColor(0x7c3aed),"8 skills, 8 agents"),
        ("foreman -> worker sessions",2026+7.65/12,2026+7.75/12,HexColor(0x0f766e),"AI supervising AI"),
        ("MCP servers",2025+4/12,2026+9/12,BLUE,"tool protocol")]),
     ("TESTING",GREEN,[
        ("Mocha (unit/system)",2024+7.3/12,2026+5/12,GREEN,"since Aug'24"),
        ("Chai + Supertest",2024+7.3/12,2026+5/12,GREEN,"assert + HTTP"),
        ("Playwright (browser E2E)",2025+5/12,2026+9/12,GREEN,""),
        ("Pact / gate / auditors",2026+3/12,2026+9/12,HexColor(0x0f766e),"contract"),
        ("Ear-check (fleet audio)",2026+7.5/12,2026+9/12,HexColor(0x0f766e),"proof in the air"),
        ("Judge panel (625 convos)",2026+7.5/12,2026+7.6/12,HexColor(0x0f766e),"AI evaluating AI")]),
     ("CI/CD, LINT & DOCS",BLUE,[
        ("GitHub Actions (5 flows)",2024+7/12,2026+9/12,BLUE,"first green Aug'26"),
        ("nodemon",2024+9.6/12,2026+9/12,HexColor(0x0369a1),""),
        ("ESLint custom rules",2026+3/12,2026+9/12,HexColor(0x0369a1),""),
        ("MkDocs",2025+4/12,2026+9/12,HexColor(0x0369a1),""),
        ("mDNS fleet discovery",2026+6.3/12,2026+9/12,HexColor(0x0369a1),"6 nodes"),
        ("KNOWN-BUGS + /log-review",2026+7.4/12,2026+9/12,HexColor(0x0369a1),"3,204 lines")])]
    ...
    for gx,gl in [(2024+7/12,"Aug'24"),(2024+10/12,"Nov'24"),(2025+1/12,"Feb'25"),
        ...
        (2026+1/12,"Feb'26"),(2026+4/12,"May'26"),(2026+7/12,"Aug'26")]:
    ...
    d.add(Rect(X(2026+4/12),Y(bottom),X(2026+6/12)-X(2026+4/12),bottom-T+8,
        fillColor=HexColor(0xf0eee9),strokeColor=None))
    for row in rows:
        if row[0]=="H":
            _,cat,col,yt=row
...
def parse_md(path):
    ...
    chart_after = {
        ...
        "Models Behind the Curtain": ("Who actually wrote it — AI trailers by named model", chart_trailers),
    }
    started=False
    for blk in blocks:
        first=blk[0]
...
def title_page():
    ...
    fl.append(Paragraph("Two years of building alongside AI — told through 2,469 commits, "
        "from a hand-typed <i>fff</i> at midnight, to an AI that obeys a constitution before it can commit, "
        "to a model whose instructions had to be <i>deleted</i> before it could do its best work. "
        "<b>Second edition, September 2026.</b>",
        S['sub']))
    fl.append(Spacer(1,0.3*inch))
    stat=lambda n,l: [Paragraph(n,S['statn']),Paragraph(l,S['statl'])]
    cells=[[Paragraph("2,469",S['statn']),Paragraph("664",S['statn']),Paragraph("13 → 89",S['statn']),Paragraph("6",S['statn'])],
           [Paragraph("commits, 25 months",S['statl']),Paragraph("AI-authored, by named model",S['statl']),
            Paragraph("avg msg length (chars/mo)",S['statl']),Paragraph("animatronics, one fleet",S['statl'])]]
