...
const HARDWARE_CONTROLLERS = {
    ...
    motor: {
        async control({ directionPin, pwmPin, leftEnablePin, rightEnablePin, direction = 'forward', speed = 50, duration = 1000 }) {
            try {
                ...
                let out;
                let mode = 'legacy';
                if (leftEnablePin != null && rightEnablePin != null && pwmPin != null) {
                    // Use dual-enable BTS7960 wrapper
                    mode = 'dual-enable';
                    const args = [String(normDir), String(speed), String(duration), String(leftEnablePin), String(rightEnablePin), String(pwmPin)];
                    console.log('[motor][BTS7960] spawn bts7960_motor_cli.py', args.join(' '));
                    out = await runWrapper('bts7960_motor_cli.py', args);
                } else {
                    // Legacy single direction pin path
                    const args = [String(normDir), String(speed), String(duration), String(directionPin), String(pwmPin)];
                    console.log('[motor][LEGACY] spawn motor_cli.py', args.join(' '));
                    out = await runWrapper('motor_cli.py', args);
                }
                ...
                return {
                    ...
                    pwmPin: pwmPin,
                    leftEnablePin: leftEnablePin,
                    rightEnablePin: rightEnablePin,
                    direction: normDir,
                    speed: speed,
                    duration: duration,
                    driverMode: mode,
        ...
        async stop({ directionPin, pwmPin, leftEnablePin, rightEnablePin }) {
            try {
                let out;
                let mode = 'legacy';
                if (leftEnablePin != null && rightEnablePin != null && pwmPin != null) {
                    mode = 'dual-enable';
                    const args = ['stop', '0', '100', String(leftEnablePin), String(rightEnablePin), String(pwmPin)];
                    console.log('[motor][BTS7960] spawn stop bts7960_motor_cli.py', args.join(' '));
                    out = await runWrapper('bts7960_motor_cli.py', args);
                } else {
                    const args = ['stop', '0', '100', String(directionPin), String(pwmPin)];
                    console.log('[motor][LEGACY] spawn stop motor_cli.py', args.join(' '));
                    out = await runWrapper('motor_cli.py', args);
                }
                const parsed = parsePythonJSON(out);
                const success = parsed ? parsed.status === 'success' : (typeof out === 'string' && out.indexOf('success') !== -1);
...
export async function controlPart(partId, action, params = {}) {
    try {
        ...
        if (type === 'motor') {
            ...
            if (part.leftEnablePin != null) normalized.leftEnablePin = Number(part.leftEnablePin);
            if (part.rightEnablePin != null) normalized.rightEnablePin = Number(part.rightEnablePin);
        }
        if (type === 'stepper') {
