...
async function getAvailableServos(characterId) {
  try {
    ...
    for (const servo of servos) {
      ...
      // An `autoGenerated` profile is a PLACEHOLDER — the store stamps every
      // uncalibrated absolute-servo with the full 0-180 mechanical span and only
      // clears the flag when a human really calibrates. Accepting it as calibration
      // means the parts.json markers fallback below never runs, so a servo with
      // hand-set Min/Max markers gets driven across its entire mechanical span
