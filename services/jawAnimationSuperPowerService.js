  ...
  try {
    ...
    if (flat.servoPartId) {
      try {
        ...
        if (part) {
          const cal = await getCalibrationForPart(part, characterId);
          if (cal.calibrated) {
 ...
 */
async function getCalibrationForPart(part, characterId) {
  try {
    const store = getCalibrationStore();
    // Profiles are keyed characterId:partId and part ids repeat across
    // characters — an unscoped lookup resolved this jaw's window against
    // whichever character the node happened to have selected.
...
async function getAvailableServos(characterId = null) {
  try {
    ...
    const servoInfo = await Promise.all(servos.map(async servo => {
      const { calibrated, minAngle, maxAngle } = await getCalibrationForPart(servo, characterId);
      return {
        ...servo,
        calibrated,
        minAngle,
...
async function loadCalibrationGuardrails(servoPartId, characterId) {
  try {
    ...
    return await getCalibrationForPart(part, characterId);
  } catch (error) {
...
async function moveJawToAngle(characterId, angleDeg) {
  try {
    ...
    // Clamp to calibrated range — through the SAME source every drive path
    // uses (profile store first, markers as fallback). Reading markers only
    // meant a character calibrated in calibration_profiles.json (Mina's jaw:
    // 22..91, no markers) got a 0..180 window here, and the Stop route's
    // moveJawToAngle(characterId, 0) drove the jaw 22° past its calibrated
...
async function playWithJawSync(characterId, audioBuffer, contentType, options = {}) {
  ...
  // Set up drive state for monitoring/cancel. jawServo/closedAngle ride along
  // so cancelJawDrive can close the mouth instead of abandoning it mid-open.
  const driveState = { cancelled: false, timer: null, amplitude: 0, angle: closedAngle, jawServo, closedAngle };
...
async function driveJawFromAudioBuffer(characterId, audioBuffer, contentType) {
  ...
  // Late-bind onto the already-registered drive state so a cancel arriving
  // from here on can close the mouth rather than abandon it.
  driveState.jawServo = jawServo;
  driveState.closedAngle = closedAngle;
...
function cancelJawDrive(characterId) {
  ...
  if (state) {
    ...
    // Close the mouth on the way out. Cancelling mid-utterance used to leave
...
async function adjustPartCalibration(partId, marker, delta, characterId) {
  ...
  // parseFloat(undefined) is NaN, NaN survives every comparison below and
  // serialises to JSON null — silently erasing a measured bound.
  if (!Number.isFinite(parseFloat(delta))) {
  ...
  // The swap above handles an INVERTED window but not an EQUAL one — the same
  // hole that froze a neck servo at {108,108} through set-min/set-max. Ten
  // clicks of "Max down" on a 10° window would pin the jaw shut forever while
  // every response reported success.
  if (isDegenerateWindow(bounds)) {
    throw new Error(
      `Cannot set ${marker} to ${newVal}°: min and max would collapse to one angle, ` +
      `locking the jaw there permanently. Widen the other bound first.`
    );
  }
