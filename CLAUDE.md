...
- **Character configuration locks:** a finished character can be FROZEN — `config/character-locks.json` + `services/characterConfigLock.js` refuse every configuration write under `data/character-{id}/` (parts, poses, scenes, super-powers, calibration, movement, gestures, queues, ai-config, images) and his registry entry, with HTTP 423 / `CHARACTER_CONFIG_LOCKED`. Runtime state (`*-state.json`, `ai_agent_state.json`) stays writable, so a locked character still runs. A deploy excludes a locked character's whole data dir. **PumpkinHead (char 1) is LOCKED** — do not "fix" his config; unlock deliberately with `node scripts/character-lock.mjs unlock 1`. `npm run lock:status` / `npm run lock:verify`. See `docs/development/CHARACTER-CONFIG-LOCKS.md`.
...
- **Dashboard (`/`):** Primary operator interface with draggable/reorderable panels. Scenes panel supports drag-reorder, delete, play individual, and loop-all. Monster Features panel has jaw/LED-Talk/head-tracking/AI-Motion toggles
...
- `POST /setup/jaw-animation/api/jaw-animation/:charId` → save jaw config (enabled, servoPartId, `ledSync`, etc.)
- **LED Animation page: `/setup/led-animation`** — the dedicated home for all `led_ring` animation (colours, palette, brightness, speech sync). Endpoints: `GET/POST /setup/led-animation/api/config/:charId` (colour block via `ledController`), `POST .../api/led-sync/:charId` (writes `jawAnimation.ledSync`), `POST .../api/test-tts/:charId` (speak & drive eyes from audio — no jaw servo needed), `POST .../api/sweep/:charId`, `POST .../api/stop/:charId`. All resolve the character through `resolveCharacter(req)`. Live colour/state control still uses `/api/led/*`.
- **LED eye sync:** `jawAnimation.ledSync` (`{enabled, partId, colorLow, colorHigh, sensitivity, smoothing, attackMs, releaseMs, speed}`, top-level, per character) makes an assigned `led_ring` react to speech. The jaw playback paths, the LED page's `playTtsWithLed`/`driveLedFromBuffer`, and the realtime agent feed the RAW audio amplitude to `services/ledSpeakingSync.js`, which applies the LED's own timing envelope (independent of the jaw) then `ledController.setState('speaking', {colorLow, colorHigh, speed})` + `pushAudioLevel(level, {replace:true})`; the daemon's `speaking` render crossfades colorLow→colorHigh by amplitude. Character-independent; no-op if the character has no `led_ring`. Configured on `/setup/led-animation`, not the jaw page.
- **LED AI-interaction states:** the conversation lifecycle drives the ring through its per-state colours — `services/ledInteractionService.setInteractionState(characterId, 'thinking'|'listening'|'idle'|'error')` (NOT 'speaking' — that's the audio-reactive path's). Hooked into `routes/conversation.js` (`/api/ask-ai` thinking→listening) and `services/elevenLabsWebSocketService.js` (thinking on user transcript, idle on session end; agent audio drives audio-reactive speaking via the ephemeral-playback RMS feed). No-op without an `led_ring`.
- **LED ring GPIO/geometry** is editable on `/setup/calibration` → Edit (data pin, PWM channel, colour order, pixel count, ring split, DMA, data rate). The save merges over existing config so colours/palette survive a geometry edit.
...
- `POST /api/orchestration/superpower/:feature` → `{ enabled, ids? }` — fleet-wide toggle (feature: lurk|jaw|head|motion|mute)
...
- `GET /api/orchestration/animatronic/:id/webcam-snapshot` → ONE JPEG frame, served from `services/mjpegFrameCache.js`. **This is what the Fleet Command Center wall uses, and the reason matters:** an MJPEG response never ends, so six live `<img>` streams permanently held all SIX of the browser's per-host HTTP/1.1 connections and every `fetch()` the page made queued behind them — the fleet pill read "1 / 6 online" while the server was answering `fleet-health` to `curl` in 0.3s. Nodes were offline only in the browser's connection queue. Snapshots release their connection on arrival. The cache keeps ONE upstream stream per node and answers from the newest frame in RAM (~20ms warm), idling out after 30s. Do NOT put endless streams back on the wall; the modal is where live video belongs, because one card is one connection.
