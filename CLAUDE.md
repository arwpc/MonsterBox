...
- **Supervised calibration override:** the calibration endpoints (nudge, home, jog-raw, and goto with `calibrationOverride: true`) relax the safety ANGLE WINDOW and DURATION CAP so an operator can measure real travel and home to a physical endstop. `blockAllMotion`, `noRetractBelowMin`, the per-part current-draw cap `safety.maxSpeedPct`
(`services/hardwareService/safetyLimits.js`), and power-group serialization NEVER relax. (The separate global speed-cap *multiplier* in the motion planner was removed in v10.1 at the operator's request — it was a timing knob that defaulted to 1.0, not a safety limit. Do not confuse the two; they used to share a name and that ambiguity caused a false safety flag.) Runtime paths (scenes, poses, superpowers, dashboards) must never set this flag.
