...
/**
 * MonsterBox 5.1 - Single Node Express Server
...
const PORT = (process.env.PORT ? parseInt(process.env.PORT, 10) : 0) || config.port || 3100;
...
app.use((req, res, next) => {
    res.renderWithLayout = function (contentTemplate, options = {}) {
        const layoutOptions = {
            title: options.title || 'MonsterBox 5.1',
...
app.get('/setup/parts', async (req, res) => {
  try {
    res.renderWithLayout('setup/parts', { title: 'Setup Parts - MonsterBox 5.1', page: 'setup-parts' });
  } catch (err) {
...
app.get('/', (req, res) => {
    res.renderWithLayout('index', {
        title: 'MonsterBox 5.1 Dashboard',
        page: 'dashboard',
        bodyExtras: `
            <script>
...
app.listen(PORT, '0.0.0.0', async () => {
    console.log(`🎭 MonsterBox 5.1 server running on port ${PORT}`);
    
    // Get primary LAN IP for display
    const ips = getLanAddresses();
    const primaryIP = ips.length > 0 ? ips[0] : 'localhost';
    
    console.log(`📱 Dashboard: http://${primaryIP}:${PORT}`);
    console.log(`⚙️  Setup: http://${primaryIP}:${PORT}/setup`);
    console.log(`🎬 Live Mode: http://${primaryIP}:${PORT}/live`);

    // LAN addresses for convenience
    try {
        if (ips.length) {
            console.log('🌐 All LAN addresses:');
            for (const ip of ips) {
    ...
    if (mjpgHealthy) {
        ...
        console.log(`🎥 Webcam streaming: http://${primaryIP}:8090/?action=stream`);
    } else {
    ...
    try {
        ...
        console.log(`🚀 Real-time AI chat: ws://${primaryIP}:8795`);
    } catch (error) {
