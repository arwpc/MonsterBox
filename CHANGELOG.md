...
- **Mina session (2026-09-21): two software defects on her node, both fixed.** The coffin door's
  ...
  homing, so every pose/scene extend was a silent no-op. Flag restored, door re-homed to p=0. Her
  tree was also ~50 files behind main (synced) and her jaw calibration had been wiped a fourth time
  (restored, jaw animation re-enabled). Signal path proven at the Pi pads with pigpio; whether the
  motor and servos move is still unwitnessed — see KNOWN-BUGS → Mina for the exact next test.
...
`docs/troubleshooting/KNOWN-BUGS.md` has the full writeup, including what is still open
(Mina's Eye still uncalibrated, a stale duplicate `parts.json`, and the `clear-all` button
itself — a distinct, still-open hazard that deletes measured calibration with no backup).

- **Fixed:** `data/calibration_profiles.json` had been reduced to six autoGenerated
  placeholders — the jaw lost its measured 28–84° window, the neck lost `capability.invert`
  **and** its 48–180° window (driving every commanded angle to the mirror image, into the
  opposite mechanical stop), and PumpkinHead's, Orlok's and Sir Dragomir's profiles were
  dropped from the file entirely. Recovered by merging a manual backup with the one profile
  calibrated since (6 → 23 profiles). Proximate cause: an operator-issued `clear-all` on the
  calibration page.
- **Fixed (test path only):** a dozen unit tests inject synthetic parts and drive the real
  calibration endpoints, snapshotting `data/calibration_profiles.json` and
  `data/actuator-positions.json` in `before()`/restoring in `after()` — a run that dies first
  (Ctrl-C, timeout, OOM, killed mocha) leaves the test's data as the node's permanent
  calibration, and `npm run gate` runs `test:smoke` on every commit and push. `MB_CALIBRATION_FILE`
  / `MB_ACTUATOR_POSITIONS_FILE` now redirect both stores, and `tests/setup.js` sandboxes both
  at a throwaway copy for the whole run.
- **Fixed:** every ± nudge answered HTTP 409 "position unknown" on every servo, permanently —
  servo position was recorded only in an in-memory Map and never persisted (regression origin
  v10.1.0). Now persisted and verified across a real service restart.
- **Fixed:** the dashboard collapsed every hardware failure (409 position-unknown, 403 safety
  refusal, 502 "GPIO busy") to the bare word "Failed". It now prints the server's own reason.
- **Fixed:** Mina's coffin door (MDD10A) could not be stopped and refused the next command for
  8.5 seconds ("GPIO busy", 54 occurrences in `monsterbox.err`) — the in-flight-drive killer
  only recognized the BTS7960 wiring shape. Both wiring shapes are matched now.
- **Fixed:** Retract reported success while commanding nothing when the door was already at
  its target; the reply now says "Already at 0 — no motion commanded".
- **Fixed:** scene step type `delay` threw "Unknown step type" and aborted the whole scene —
  Mina's scene 100 died on its second step every play. `delay` is now an alias for `wait`.
- **Fixed:** every ordinary servo (declared `rotationRangeDeg: 180`, the standard hobby-servo
  range) was misclassified as multi-turn in the batch sync path, so multi-part poses lost
  synchronisation. The batch path now agrees with the single-part dispatch logic.
...
a number it had been *given* rather than the one it had *used*. A servo said it moved
to 60 while the register held 119.6. A nudge said "fine" and jumped 43 degrees. A
camera panel said "Streaming" over a dead socket. A settings page offered five sliders
that reached nothing at all. None of these were crashes; every one of them sent
somebody to debug working hardware.

### Mina's servos are not on the channels anyone thought — and it is not a mapping bug

The live question was whether Mina's servos sit on PCA9685 channels 0/4/9/11 rather
than the 4/8/11 in `parts.json`. All sixteen channels were swept with two independent
witnesses. **Channel 4 (the Jaw) is the only channel with anything responding on it.**
9, 11 and 12 are as silent as the channels nothing was ever wired to, so this is
neither the guessed map nor an off-by-one.

The software is exonerated by the same measurement: the register sampler saw correct
PWM go out on every silent channel while the jaw, on the same chip and the same V+
rail, rose +18 dB. `parts.json` needs no remap; the fault is on the far side of the
header. `docs/hardware/PCA9685-CHANNEL-MAP-MINA.md` has the per-channel numbers and
`docs/hardware/OPERATOR-TODO.md` has the three physical checks, in order.

Getting a trustworthy answer meant fixing the *instrument* twice, which is worth
recording because both failures produce a confident, wrong "everything is dead":
ALSA `default` on this node routes to a capture input that reads room tone 28 dB
quieter than the working microphone, and scoring band energy per segment instead of
per fixed-length frame made an idle control look like a moving servo. A measurement
rig needs a positive control and an idle control before any silence means anything.

**A PCA9685 cannot report what is attached to it.** It is a one-way I²C→PWM driver and
a servo has no back-channel. Driving a channel and observing is the only method there
will ever be. USB devices are the opposite case, which is why they now carry identity.

### The smallest control on the calibration page was the dangerous one

`nudge` began from a fabricated 90°. A jaw physically at 131.5° took a "fine" nudge,
was told "Nudged min at fine", and drove to 88° — below its calibrated minimum of 97°,
because nudge is deliberately bounds-free. It now seeds from the position store and
**refuses** when the position is unknown, rather than guessing.

That refusal exposed a worse one underneath. The first refusal message advised "set
min/max first" — and Set Min substituted 0° when position was unknown, then stamped
the profile `autoGenerated: false`, recording a guess *as a measurement*. On this node
that would have replaced Mina's measured `{22, 91}` with `{0, 91}` and quietly allowed
22° of travel past the jaw's real floor. Both branches now refuse too.

Also: homing overdrive no longer applies to ordinary moves (a "nudge by −0.05" landing
on p=0 became a 2.75 s full-force drive into the endstop), and `durationMs` is finally
honoured — but capped at the 30 s ceiling jog-raw already used. Reading the caller's
duration was the fix; obeying it without bound would have let a 0.01 nudge run the
motor for ten minutes.

### Dead controls, removed rather than explained

The operator's standard: *"it's worse than no functionality at all."*

The STT Sample Rate dropdown offered five rates while all four capture sites hardcode
16000/mono, and its only apparent reader was a validator with zero callers anywhere —
which is exactly why it had survived earlier greps looking alive. The five "Buffer
Timing" sliders auto-saved to disk and nothing ever read them. All gone.

One nuance worth stating: there were **two** microphone gain sliders on that page and
only one was dead. `#inputGain` really moves PipeWire; `#microphoneInputGain` only
wrote a key. The decoy is gone and the working one stays.

### The cameras were always fine

Orlok's stream serves 933 KB in 6 s. Every "the camera is broken" report was the UI. In
the Fleet Command Center a single transient error latched a healthy webcam at "Webcam
unavailable" permanently, because the retry guard could never become true again. On the
dashboard, the opposite lie: "Streaming" with no error handler at all, so a 503 looked
identical to a working feed.

USB hardware now carries a model signature — `meta.usbId` holding `idVendor:idProduct`
plus descriptor strings, at the manufacturer/model level the operator asked for and
explicitly **not** the per-unit serial, because a serial would make an identical
replacement camera read as the wrong device. Both signatures were read off live
hardware and checked against `lsusb`; nothing was guessed.

### The browser suite was moving the coffin

507 passing, 0 failing, 8 skipped — from a 501/6/8 baseline. Three of the six briefed
failures never reproduced (timing flake in a 13.5-minute file); three were real.

The most interesting was `scene-concurrency`, which asked the play endpoint to run a
real scene with a 10-second budget against a ~19-second scene. It failed on the clock,
but while failing it drove the coffin actuator for 8.5 s twice and called the TTS API,
on every suite run, because port 3100 runs with `MB_TEST_MODE` unset. It now uses the
route's existing `?dryRun=1`.

The mobile-viewport failure turned out to be a genuine responsive bug rather than a
test artefact: at 375 px the fixed control bar overlays the deck tablist, so a click on
the AI tab hit-tests to the PANIC button.

### Reverted before shipping: persistent armed superpowers

Head tracking, the PIR watcher and the idle loop each keep "is it on" in a module
binding, so a restart silently disarms them while the UI still reads on. A fix for that
was written and then **removed before release**, because an adversarial review found it
was half inert and half dangerous: the head-tracking half never persisted at all — every
caller passes a null `characterId`, which the live server confirmed in its own log — and
the idle-loop half worked, meaning a node would **start driving servos unattended on
boot**. In a house where animatronics have already woken people at 00:20, that is not a
trade worth making for a state flag. The defect stays open and documented; the design it
needs is recorded with it.

### Somebody is watching the chain now

`docs/development/PART-MODEL-CALIBRATION-UX-CHAIN.md` answers a question the operator
asked directly. It records six hops, six invariants, and the measured shape divergences
between characters — including two corrections that have each already misled more than
one investigation: **`invert` is not a part-record field** (it lives on the calibration
profile's `capability`), and `deviceId` is a *number* on Orlok and a *string* on the
other two.

The dominant defect it found is a class, not an instance: `characterId` is resolved
correctly at one hop and dropped at the next, and part ids repeat across characters, so
an unscoped lookup addresses a different animatronic's hardware. `audit:resolver` cannot
catch it, because the read is legal — it is the passing-on that goes missing. One
instance was destructive and is fixed: `POST /clear-all` accepted a `characterId`,
ignored it, and wiped *this* node's calibration while reporting success. The rest are
listed in KNOWN-BUGS as open, with file:line, rather than quietly half-fixed.

### Also

- `webcam_cli.py` accepts every spelling of `deviceId` that actually appears in the
  fleet's data (`0`, `"0"`, `"video0"`, `/dev/video0`). Mina had logged 25 identical
  hardware errors against a working camera because `int("video0")` throws.
- `CLAUDE.md` stopped promising a safety key that no longer exists: `safety.maxSpeedPct`
  matches no source file, so adding it to protect a circuit would have enforced nothing.
- `scripts/verify-deploy.sh` greps each node for shipped symbols, because a node's
  `/health` version, its `package.json` and its git HEAD are all worthless as evidence —
  two nodes serve 10.1.0 from a checkout whose HEAD reads v9.3.0 with 32 dirty files.

**Nothing in this release was verified by ear.** Speaker output was off all session by
operator instruction, and every node was confirmed `muted: true` before and after.
