...
router.post('/:partId/calibrated', express.json(), async (req, res) => {
  try {
    ...
    if (calibrated && profile.capability && profile.capability.kind === 'absolute-servo') {
      ...
      if (profile.autoGenerated && b.minAngle === 0 && b.maxAngle === 180) {
        warning = 'Stamped, but this servo still carries the full 0-180 default span — ' +
...
router.delete('/:partId/profile', async (req, res) => {
  try {
    ...
    // loss of Orlok part 15's measured window was undiagnosable because this
