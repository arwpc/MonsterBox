...
async function getOrAutoCreateProfile(partId) {
  ...
  if (profile) {
    ...
    // rotationRangeDeg meant anything). Only the capability is synced; measured
    // bounds are the operator's and their units cannot be guessed — but a
    // PLACEHOLDER span is refreshed to the real range so the page shows the
    // truth it is about to measure against.
    if (profile.capability && profile.capability.kind === 'absolute-servo') {
      try {
        const parts = await loadParts();
        const part = parts.find(p => String(p.id) === String(partId));
        ...
        if (declared !== stored) {
          ...
          if (profile.autoGenerated) profile.bounds = { minAngle: 0, maxAngle: declared };
