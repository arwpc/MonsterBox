...
class FileManager {
   ...
   */
  async scanDirectoryRecursive(dirPath, type, prefix = '') {
    const files = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const subFiles = await this.scanDirectoryRecursive(fullPath, type, relativePath);
          files.push(...subFiles);
        } else if (entry.isFile() && this.isSupportedFormat(entry.name, type)) {
  ...
  async scanMediaFiles() {
    ...
    // Scan regular media directories recursively
    for (const [type, dirPath] of Object.entries(this.mediaPaths)) {
      try {
        const files = await this.scanDirectoryRecursive(dirPath, type);
        mediaList[type].push(...files);
        console.log(`📁 Scanned ${type} directory: found ${files.length} files`);
      } catch (error) {
