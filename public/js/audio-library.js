...
(function() {
    ...
    function renderTable() {
        ...
        for (var i = 0; i < audioFiles.length; i++) {
            ...
            var rowClass = isPlaying ? 'is-playing mb-live' : '';
            ...
            html += '<tr class="' + rowClass + '" data-audio-id="' + escapeAttr(audio.id) + '">'
                ...
                + '<td class="mb-text-muted mb-text-sm mb-mono">' + formatDuration(audio.duration) + '</td>'
                + '<td class="mb-text-muted mb-text-sm mb-mono">' + escapeHtml((audio.format || '').toUpperCase()) + '</td>'
                + '<td class="mb-text-muted mb-text-sm mb-mono">' + formatFileSize(audio.fileSize || 0) + '</td>'
                + '<td>'
    ...
    function createEditModal() {
        var modalHTML = '<div class="modal fade" id="editAudioModal" tabindex="-1">'
            ...
            + '<button type="button" class="btn-close" data-bs-dismiss="modal" title="Close without saving" aria-label="Close without saving"></button>'
            + '</div>'
            + '<div class="modal-body">'
            + '<form id="editAudioForm">'
            + '<div class="mb-field">'
            + '<label class="mb-field-label" for="editTitle">Title</label>'
            + '<input type="text" class="mb-input" id="editTitle" required title="Display title for this audio file">'
            + '</div>'
            + '<div class="mb-field">'
            + '<label class="mb-field-label" for="editDescription">Description</label>'
            + '<textarea class="mb-textarea" id="editDescription" rows="3" title="Optional description"></textarea>'
            + '</div>'
            + '<div class="mb-field">'
            + '<label class="mb-field-label" for="editCategory">Category</label>'
            + '<select class="mb-select" id="editCategory" title="Category for this audio file"></select>'
            + '</div>'
            + '<div class="mb-field">'
            + '<label class="mb-field-label" for="editTags">Tags (comma separated)</label>'
            + '<input type="text" class="mb-input" id="editTags" placeholder="spooky, halloween, monster" title="Comma-separated tags">'
            + '</div>'
            + '<div class="mb-field">'
            + '<label class="mb-check" for="editFavorite">'
            + '<input type="checkbox" id="editFavorite" title="Mark this file as a favorite">'
            ...
            + '<button type="button" class="mb-btn mb-btn-secondary" data-bs-dismiss="modal" title="Discard changes">Cancel</button>'
            + '<button type="button" class="mb-btn mb-btn-primary" id="editSaveBtn" title="Save changes to this audio file">Save Changes</button>'
            + '</div>'
            + '</div>'
            + '</div>'
            + '</div>';
    ...
    function displaySelectedFiles() {
        ...
        for (var i = 0; i < selectedFiles.length; i++) {
            ...
            html += '<div class="d-flex justify-content-between align-items-center py-1">'
                ...
                + '<button class="mb-btn mb-btn-sm mb-btn-danger mb-btn-icon remove-file-btn" title="Remove this file from the upload" data-index="' + i + '">'
